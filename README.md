...
| `additionalTargets`    | optional, array of additional hostnames/IP addresses/MAC addresses for the *same* person (e.g. a smartwatch in addition to a phone). The sensor is active if **any** of `target` + `additionalTargets` was seen within `threshold`.  |
| `customDns`            | if `target` is a hostname, you can optionally set an array of custom DNS servers here to use for the hostname lookup. Applies to all of this sensor's targets.                                     |
